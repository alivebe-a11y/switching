...
def test_simulate_and_summarize(monkey_get_history):
    ...
    trades = backtest.simulate(signals, hold_days=5, cost_bps=0.0, entry_delay=0)
    assert len(trades) == 3
...
def test_simulate_applies_cost(monkey_get_history):
    ...
    trades = backtest.simulate(signals, hold_days=5, cost_bps=50.0, entry_delay=0)  # 50 bps
    assert len(trades) == 1
...
def test_simulate_respects_min_severity(monkey_get_history):
    ...
    trades = backtest.simulate(signals, hold_days=5, min_severity=0.5, entry_delay=0)
    assert len(trades) == 1
...
def test_stop_loss_exits_early(monkey_exit_history):
    ...
    trades = backtest.simulate([sig], hold_days=5, cost_bps=0, stop_loss=0.05, entry_delay=0)
    assert len(trades) == 1
...
def test_take_profit_exits_early(monkey_exit_history):
    sig = _sig("TEST", monkey_exit_history)
    trades = backtest.simulate([sig], hold_days=5, cost_bps=0, take_profit=0.05, entry_delay=0)
    assert len(trades) == 1
...
def test_first_green_exits_on_first_positive_close(monkey_exit_history):
    sig = _sig("TEST", monkey_exit_history)
    trades = backtest.simulate([sig], hold_days=5, cost_bps=0, first_green=True, entry_delay=0)
    assert len(trades) == 1
...
def test_first_green_with_stop_loss(monkey_exit_history):
    ...
    trades = backtest.simulate(
        [sig], hold_days=5, cost_bps=0, first_green=True, stop_loss=0.05, entry_delay=0,
    )
    assert len(trades) == 1
...
def test_hold_when_no_exit_strategy(monkey_exit_history):
    sig = _sig("TEST", monkey_exit_history)
    trades = backtest.simulate([sig], hold_days=5, cost_bps=0, entry_delay=0)
    assert len(trades) == 1
