<!--
Source: agent-rules-books by ciembor — https://github.com/ciembor/agent-rules-books
File:   release-it/release-it.mini.md  (MIT licensed)
Distilled from "Release It!" by Michael T. Nygard.

Why it's here: this bot is almost entirely flaky-external-dependency plumbing
(yfinance, Trading 212, RNS scraping, Telegram, SEC EDGAR). Release It! is the
book about exactly that. Imported into CLAUDE.md so these rules are active when
adding any outbound call, queue, cache, scheduled job, or deploy/ops path.
See CLAUDE.md "Stability rules" for how these map onto our code.
-->
